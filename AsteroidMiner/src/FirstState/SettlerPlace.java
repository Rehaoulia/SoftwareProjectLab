...
public class SettlerPlace extends AbstractAppState  {
 ...
 AmbientLight amb = new AmbientLight();
    ...
    public Node loadAsteroids() {

        Node base = new Node("Asteroids");
        Spatial asteroid = assetManager.loadModel("Models/AsteroidModel/LowP0002.j3o");

        Random rand = new Random();
        Asteroid[] ag = new Asteroid[1000];

        for (int i = 0; i < 1000; i++) {

            int max = 400;

            int randx = ThreadLocalRandom.current().nextInt(-max, max);
            int randy = ThreadLocalRandom.current().nextInt(-max, max);
            int randz = ThreadLocalRandom.current().nextInt(-max, max);
            ...
            if (mineralSelector == 4)
                ag[i] = new Asteroid(i, GenLoc, radius);
            else
                ag[i] = new Asteroid(i, GenLoc, M, radius);

            Spatial n = asteroid.clone();
            n.setLocalTranslation(GenLoc);
            n.setLocalScale(randSize / 50);
            ag[i].setModel(n);

            base.attachChild(ag[i].getModel());

        }

        AmbientLight amb = new AmbientLight();
        amb.setColor(new ColorRGBA(1.0f, 1.0f, 1.0f, 0.001f));
        base.addLight(amb);
