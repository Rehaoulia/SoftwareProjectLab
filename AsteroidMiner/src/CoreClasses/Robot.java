...
public class Robot extends Traveler implements Craftable {
	...
	public void travel(Asteroid destination) {
		...
		System.out.println("robot traveled to asteroid"+currentAsteroid.getID());
	}
	...
	public void drill() {
		...
		if (currentAsteroid.destroyed() == true) {
			// travel(Controller.getClosestAsteroid(currentAsteroid)); //this function does
			// not exist yet
		}
	}
	...
	public boolean behave(Asteroid ast) {
	...
	public void hide() {
		try {
			if ((currentAsteroid.hollow()) && (currentAsteroid.getDepth() == currentAsteroid.radius)) {// depth is
																										// private
				setHidden(true);
